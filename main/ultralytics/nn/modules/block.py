...
class MASPPF(nn.Module):
    ...
    def forward(self, x):
        ...
        # combined = self.cv3(combined)
        # y.extend(combined)
        #
        # # 重复三次并行结构
        # for _ in range(2):  # 已经进行了第一次，所以只重复两次
        #     max_pooled = self.max_pool(combined)
        #     avg_pooled = self.avg_pool(combined)
        #     combined = torch.cat((max_pooled, avg_pooled), dim=1)
        #     combined = self.cv3(combined)
        #     y.extend(combined)
        #
