...
__all__ = ['DyHeadBlock', 'DyHeadBlockWithDCNV3', 'Fusion', 'FG_C2f','FGBottleneck','C2f_Faster', 'C3_Faster', 'C3_ODConv', 'C2f_ODConv', 'Partial_conv3', 'C2f_Faster_EMA', 'C3_Faster_EMA', 'C2f_DBB',
           ...
           'C3_ContextGuided', 'C2f_ContextGuided', 'C3_MSBlock', 'C2f_MSBlock', 'ContextGuidedBlock_Down', 'C3_DLKA', 'C2f_DLKA', 'CSPStage', 'SPDConv',"SPDBottleneck","SPD_C2f",
           ...
           'C2f_AdditiveBlock', 'C2f_AdditiveBlock_CGLU', 'CSP_MSCB', 'EUCB', 'C2f_MSMHSA_CGLU', 'CSP_PMSFA','MXSPDConv']
...
class FG_C2f(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        print(f"c1: {int((2 + n) * self.c)}, c2: {c2}")
        self.cv2 = Conv(int((2 + n) * self.c), c2, 1)  # optional act=FReLU(c2)
        self.m = nn.ModuleList(FGBottleneck(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))
...
class MXSPDConv(nn.Module):
    def __init__(self, inc, ouc, dimension=1):
        super().__init__()
        self.d = dimension
        self.cv1 = Conv(inc * 4, ouc, k=3)
        self.cv2 = Conv(inc * 4, ouc, k=5)
        self.cv3 = Conv(inc * 4, ouc, k=7)
        self.cv4 = Conv(3 * ouc, ouc, 1)
...
class SPD_C2f(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        self.m = nn.ModuleList(SPDBottleneck(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))

    # def forward(self, x):
    #     print(f"X: {x.size()}")
    #     x1 = self.cv1(x)
    #     print(self.cv1)
    #     print(f"Size after cv1: {x1.size()}")
    #     x2 = self.cv2(x1)
    #     print(f"Size after cv2: {x2.size()}")
...
class SPDBottleneck(nn.Module):
    ...
    def forward(self, x):
        x1 = self.cv1(x)
        x2 = self.cv2(x1)
        print(f"x size: {x.size()}, x1 size: {x1.size()}, x2 size: {x2.size()}")
        if self.add:
            x = self.cv3(x)  # 通过 1x1 卷积进行升维
            return x + x2
        else:
            return x2
